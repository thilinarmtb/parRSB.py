...
def conn_mesh(cnp.ndarray[cnp.float64_t, ndim=2] coord,
    ...
    cdef int ndof = coord.shape[0]
    cdef int ndim = coord.shape[1]
    ...
    cdef int nv = 8
    cdef int nelem = ndof // nv

    cdef double *coord_ = <double *>malloc(nelem * ndim * cython.sizeof(cython.double))
    ...
    cdef long long *vtx_ = <long long *>malloc(nelem * nv * cython.sizeof(cython.longlong))
